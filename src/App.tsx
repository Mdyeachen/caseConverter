    ...
    <div className="bg-slate-200 min-h-screen w-full mx-auto flex flex-col items-center justify-start p-6 gap-6 font-sans">
      ...
      <div className="w-full flex gap-6 justify-center items-start flex-col md:flex-row">
        ...
        <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-xl border border-slate-200">
          <h1 className="text-3xl font-black text-slate-800 uppercase mb-2 text-center tracking-tight">Case <span className="text-amber-500">Converter</span></h1>
          ...
          <div className="flex justify-between px-4 py-3 bg-slate-50 rounded-xl border-x border-b border-slate-200 text-xs font-mono uppercase text-slate-500 mt-2">
            <span>Characters: <b className="text-slate-800">{charCount}</b></span>
            <span>Words: <b className="text-slate-800">{wordCount}</b></span>
          </div>

          {/* button */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-6">
            <button onClick={handleSentenceCase} className="btn-case">Sentence case</button>
            <button onClick={handleLowerCase} className="btn-case">lower case</button>
            <button onClick={handleUpperCase} className="btn-case">UPPER CASE</button>
            <button onClick={handlerCapitalizeCase} className="btn-case">Capitalize Word</button>
          </div>

          {/* copy and clear textarea */}
          <div className="flex gap-3 mt-4 border-t pt-4 border-slate-100">
            <button onClick={handlerCopy} className="flex-1 bg-slate-800 text-white py-2.5 rounded-lg font-semibold hover:bg-slate-700 transition-all active:scale-95 shadow-md">
              Copy Text
            </button>
            <button onClick={handlerClear} className="flex-1 border-2 border-slate-200 text-slate-500 py-2.5 rounded-lg font-semibold hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all active:scale-95">
              Clear
            </button>
          </div>
        </div>
        ...
        <div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl border border-slate-200">
          <h1 className="text-xl font-black text-slate-800 uppercase mb-2 text-center tracking-tight"><span className="text-amber-500">Duplicate</span> Words</h1>
            {duplicateWords.length > 0 ? (
            <div className="flex flex-wrap gap-2 py-4">
              {duplicateWords.map(([word, count]) => (
              <span key={word} className="border border-slate-200 p-2 rounded">
                <span className="word">{word} </span>
                <span className="count text-xs p-0.5 bg-amber-500 text-white rounded">{count}</span>
            </span>
              ))}
            </div>
          ) : (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 text-center px-4">
              ...
              <p className="text-sm italic">No duplicate words detected yet.</p>
            </div>
          )}
        </div>
      </div>
